 ...
 */
// @ts-nocheck - raw snake_case purchase fixtures mirror the sibling harness
...
const PREFS_LOADED = { remoteValues: {} };
const PREFS_NOT_LOADED = { remoteValues: null };
const dismissedPrefs = ( ...ids ) => ( {
	remoteValues: {
...
function renderReminder( {
	...
	const result = render(
		<ReduxProvider store={ store }>
			<UpcomingRenewalsReminder cart={ cart } addItemToCart={ addItemToCart } />
		</ReduxProvider>
	);
	return { ...result, addItemToCart };
}
...
describe( 'UpcomingRenewalsReminder', () => {
	...
	describe( 'urgent auto-open', () => {
		...
		test( 'lists only the urgent subset, not the non-urgent renewable purchases', async () => {
			...
			expect( await screen.findByText( 'Upcoming renewals' ) ).toBeVisible();
			expect( screen.getByText( URGENT_DOMAIN_NAME ) ).toBeVisible();
			expect( screen.queryByText( NON_URGENT_PLAN_NAME ) ).not.toBeInTheDocument();
		} );
		...
		test( 'fires the impression event once on auto-open', async () => {
			...
			const impressions = recordTracksEvent.mock.calls.filter(
				( [ name ] ) => name === 'calypso_checkout_urgent_renewals_modal_impression'
			);
	...
	describe( 'regressions', () => {
		...
		test( 'the manual link lists all renewable purchases, diverging from the urgent path', async () => {
			...
			expect( await screen.findByText( 'Upcoming renewals' ) ).toBeVisible();
			expect( screen.getByText( URGENT_DOMAIN_NAME ) ).toBeVisible();
			expect( screen.getByText( NON_URGENT_PLAN_NAME ) ).toBeVisible();
		} );
	} );
} );
