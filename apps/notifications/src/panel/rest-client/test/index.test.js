...
// Mirrors settings.max_limit in the client: the total number of notes the panel
// pages up to. Kept here so the cap-boundary tests track a change to that value.
const MAX_LIMIT = 200;
...
describe( 'RestClient', () => {
	...
	describe( 'load-more pagination', () => {
		...
		it( 'requests the final slot plus the echoed anchor when one short of the cap', () => {
			...
			// the anchor back, so the page is sized to the remaining slot plus one for
			// that anchor (number=2) — otherwise the lone returned note is the anchor
			// and the view stalls one short instead of filling to the cap.
		...
		it( 'latches when a capped page returns only the echoed anchor', () => {
			// Same one-short-of-cap setup, but the server is genuinely exhausted: the
			// page comes back with just the anchor (a short page, 1 of 2). The no-new
			// guard must latch so the catch-up loop settles instead of refetching.
			// ids (MAX_LIMIT)..2, oldest is id 2.
			store.dispatch( actions.notes.addNotes( fullPage( MAX_LIMIT - 1, MAX_LIMIT ) ) );
		...
		// End-to-end against a server with effectively unlimited notes and an
		// inclusive `before` cursor (the production WP.com behavior): each older page
		// echoes the anchor back. The window must page all the way to the cap rather
		// than stalling short once the echoed anchor is double-counted.
